...
export function useTransactionSideEffects() {
  ...
  const engineResult = useDfmEngine();
  ...
  // Record DFM history once per day
  useEffect(() => {
    ...
    const alreadyRecorded = state.dfmHistory.some(h => h.date === todayKey);
    if (alreadyRecorded) return;

    dispatch({
      type: 'RECORD_DFM_HISTORY',
      payload: { date: todayKey, dfm: engineResult.dfm.dailyFreeMoney },
    });
  }, [engineResult]); // eslint-disable-line react-hooks/exhaustive-deps
}
